...
export async function addNewContractor(data) {
  try {
    ...
    const resUser = await axios.post(
      server + `/api/users`,
      {
        username:
          data.type === 1
            ? `${data.inn}-${data.kpp}`
            : `${data.inn}-${data.ogrnip}`,
        email:
          data.type === 1
            ? `${data.inn}@${data.kpp}.ru`
            : `${data.inn}@${data.ogrnip}.ru`,
    ...
    // -------------------------------------------------------
    if (resUser.data) {
      const resContractor = await axios.post(
        server + `/api/contractors`,
        {
          data: {
            name: data.name,
            inn: data.inn,
      ...
      const resContract = await axios.post(
        server + `/api/contracts`,
        {
          data: {
            ...
            dateEndContract: dayjs(data.dateEndContract).add(1, "day"),
...
export async function changePurposeInContract(idContract, newPurposeId) {
  try {
    const res = await axios.put(
      server + `/api/contracts/${idContract}`,
      {
        data: {
